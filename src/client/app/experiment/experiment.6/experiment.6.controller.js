...
function Experiment6Controller($scope, $location, experimentDataservice, userDataservice, productDataservice){
		...
		for(var i = 0; i < $scope.products.length; i++){
			if($scope.products[i].productId == productId){
				if($scope.products[i].choosedIn6 === false){
					if(choosedIn6.length > 0){
						choosedIn6.splice(0,1);
					}
					choosedIn6.push($scope.products[i].name);
					$scope.products[i].choosedIn6 = true;
				}else{
					for(var j = 0; j < choosedIn6.length; j++){
						console.log(choosedIn6[j]);
						console.log($scope.products[i].name);
						if(choosedIn6[j] == $scope.products[i].name){
							choosedIn6.splice(j,1);
    ...
    */
    function updateUser(userId) {
        var user = new User(userId, $scope.visitedIn2, $scope.choosedIn3, $scope.visitedIn5, choosedIn6);
